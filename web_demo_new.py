...
from camera_path_solver import camera_path_solver, CauchyLoss
...
async def main(config):
  ...
  solver = camera_path_solver(image_width, image_height,
                      target_width, target_height, first_point=0.5)
  ...
  while cap.isOpened():
      ...
      if (wh_flag == 0):
        ...
        optimal_x_center = solver.compute_camera_path(optimal_x_center)

        if (optimal_x_center * image_width + target_width // 2 > image_width):
          optimal_x_center = (image_width - target_width / 2) / image_width

        if (optimal_x_center * image_width - target_width // 2 < 0):
          optimal_x_center = (target_width / 2) / image_width
        ...
        optimal_y_center = solver.compute_camera_path(optimal_y_center)

        if (optimal_y_center * image_height + target_height // 2 > image_height):
          optimal_y_center = (image_height - target_height / 2) / image_height

        if (optimal_y_center * image_height - target_height // 2 < 0):
          optimal_y_center = (target_height / 2) / image_height
