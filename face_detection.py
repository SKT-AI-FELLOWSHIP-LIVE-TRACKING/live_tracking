...
class face_detection:
        ...
        if self.detections:
            for detection in self.detections:
                ...
                for i in range(4):
                    ...
                    region = self.ExtendSalientRegionWithPoint(keypoint.x, keypoint.y, region)
    ...
    def ExtendSalientRegionWithPoint(self, x, y, region):
        if(region.w is None):
            region.set_w(self.NormalizeX(1))
        elif(x < region.x):
        ...
        if(region.h is None):
            region.set_h(self.NormalizeY(1))
        elif(y < region.y):
    ...
    def NormalizeX(self, pixel):
        print(pixel / float(self.image_width))
        return pixel / float(self.image_width)

    def NormalizeY(self, pixel):
